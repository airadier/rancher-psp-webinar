...
$ k create -f pod-privileged.yaml
$ k attach -ti pod sudo-test
```
...
But how do we map which pods can use which PSPs? Let's move on
...
Show kube-apiserver-minikube pod in kube-system is not able to start after we enabled PSPs

We need to allow the user system:node:minikube (the user which kubelet authenticates as) to use privileged PSP
...
so we are using user *minikube-user* in group *system:masters* which is binded to cluster-admin ClusterRole, allowing us to USE any PSP on creation.
